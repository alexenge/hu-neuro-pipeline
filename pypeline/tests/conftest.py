...
@pytest.fixture(scope='session')
def sample_input_pipeline(sample_data):
    """Creates and runs an InputPipeline for the sample data."""

    input_pipeline = InputPipeline(raw_file=sample_data['raw_files'][0],
...
@pytest.fixture(scope='session')
def sample_input_pipeline_besa(sample_data):
...
@pytest.fixture(scope='session')
def sample_preprocessing_pipeline(sample_input_pipeline):
    """Creates and runs a PreprocessingPipeline for the sample data using
    ICA."""

    preprocessing_pipeline = PreprocessingPipeline(downsample_sfreq=100.0,
                                                   bad_channels='auto')
...
@pytest.fixture(scope='session')
def sample_preprocessing_pipeline_besa(sample_input_pipeline_besa):
...
@pytest.fixture(scope='session')
def sample_participant_pipeline(sample_data,
                                sample_preprocessing_config,
                                sample_epoching_config):

    participant_pipeline = ParticipantPipeline(raw_file=sample_data['raw_files'][0],
                                               log_file=sample_data['log_files'][0],
